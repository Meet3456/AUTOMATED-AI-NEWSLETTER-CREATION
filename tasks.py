...
class AINewsLetterTasks():
  ...
  def fetch_news_task(self,agent):
    return Task(
      description=f'Fetch top AI news stories from the past 48 hours. The current time is {datetime.now()}.',
      ...
      expected_output=
      """
        A list of top AI news story titles, URLs, and a brief summary for each story from the past 48 hours and the respective applications of each of the articles
        ...
        [
          {  
            ...
            'applications' : 'develope llm's , finetune llm's'
          }, 
          {{...}}        
        ]
  ...
  def analyze_news_task(self, agent, context):
      return Task(
          description='Analyze each news story fetched and ensure there are at least 5 well-formatted articles',

          agent=agent,

          async_execution=True,

          # Information From the previous tasks is passed as context to the current task
          context=context,

          expected_output=
          """
            A markdown-formatted analysis for each news story, including a rundown, detailed bullet points, and a "Why it matters" section. There should be at least 5 articles, each following the proper format.
